naissance.GeometryImage = class extends naissance.Geometry {
	...
	draw() {
		...
		if (!derender_geometry) {
			try {
				...
				if (!this.geometry) {
					this.geometry = new maptalks.ui.UIMarker(coords_obj.center, { draggable: false, single: false, content: this.dom_wrapper });
	...
	handleMouseDown(e) {
		...
		this._is_dragging = false;
		if (this.selected_point_index === null) {
			let source_x = event_pos.x, source_y = event_pos.y;
	...
	render() {
		...
		this.ctx.save();
		let render_scale = this.canvas.width / this.world_size, warp_mode = this.value[1]?.warp_mode || "triangulation";
		...
		if (warp_mode === "tps" && this.mesh_points.length >= 3) {
			...
			for (let i = 0; i < this.mesh_triangles.length; i += 3) {
				let p1 = this.mesh_points[this.mesh_triangles[i]], p2 = this.mesh_points[this.mesh_triangles[i + 1]], p3 = this.mesh_points[this.mesh_triangles[i + 2]];
		...
		if (this.selected) Geospatiale.drawMeshOverlay(this.ctx, this.mesh_points, this.mesh_triangles, this.getScaleFactor(), this.base_point_radius, this.selected_point_index);
		this.ctx.restore();
		this.updateInfoPanels();
	}
	
	updateBufferSize() {
		let factor = this.getScaleFactor(), dpr = window.devicePixelRatio || 1, padding = this.base_screen_padding / factor;
		let min_x = this.img_center, max_x = this.img_center, min_y = this.img_center, max_y = this.img_center;
		this.mesh_points.forEach((p) => { min_x = Math.min(min_x, p.x); max_x = Math.max(max_x, p.x); min_y = Math.min(min_y, p.y); max_y = Math.max(max_y, p.y); });
		let max_ext = Math.max(Math.abs(min_x - this.img_center), Math.abs(max_x - this.img_center), Math.abs(min_y - this.img_center), Math.abs(max_y - this.img_center));
		this.world_size = Math.ceil((max_ext + padding) * 2);
		this.buffer_scale = Math.min(factor, this.max_buffer_size / (this.world_size * dpr));
		let target_sw = Math.ceil(this.world_size * this.buffer_scale * dpr);
		if (this.canvas.width !== target_sw) { this.canvas.width = target_sw; this.canvas.height = target_sw; }
	...
	updateCSSSize() {
		...
		let factor = this.getScaleFactor(), css_size = Math.round(this.world_size * factor);
		...
		let map_bearing = map.getBearing(), map_pitch = map.getPitch(), map_size = map.getSize();
		let fov = map.getFov ? map.getFov() : 36.86989764584402;
		
		// 1. Perspective calculation using FOV: distance = (height / 2) / tan(fov / 2)
		let perspective_dist = (map_size.height / 2) / Math.tan((fov / 2) * Math.PI / 180);
		
		// 2. Identify camera vanishing point relative to this marker
		...
		// 3. Apply perspective to container
		...
		// 4. Ground-align rotation (RotateZ first to align axes, then RotateX to tilt)
