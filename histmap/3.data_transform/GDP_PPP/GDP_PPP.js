global.GDP_PPP = class {
	...
	static async A_scaleGDP_PPPRastersToNational () {
		//Declare local instance variables
		let hyde_years = landuse_HYDE.sorted_hyde_years;
		...
		for (let i = 0; i < hyde_years.length; i++) {
			let local_ols_file_path = `${this.intermediate_normalised_to_global}GDP_PPP_${hyde_years[i]}.png`;
			...
			let local_output_file = `${this.intermediate_scaled_to_national}GDP_PPP_${hyde_years[i]}.png`;
			
			//1. Operate over file; populate local_gdp_ppp_sums; calculate local_gdp_ppp_scalars
			...
			Object.iterate(local_gdp_ppp_sums, (local_key, local_value) => {
				...
				if (local_actual_gdp_ppp) {
					...
					local_gdp_ppp_scalars[local_key] = 1;
				}
			});
			console.log(`- Local OLS object:`, local_gdp_ppp_sums);
			console.log(`- Local OLS scalars:`, local_gdp_ppp_scalars);
			
			//2. Scale by local_gdp_ppp_scalars
	...
	static async B_scaleGDP_PPPRastersToGlobal (arg0_input_folder, arg1_output_folder) {
		...
		//Iterate over all hyde_years
		for (let i = 0; i < hyde_years.length; i++) {
			let local_ols_file_path = `${input_folder}GDP_PPP_${hyde_years[i]}.png`;
			if (!fs.existsSync(local_ols_file_path)) continue; //Guard clause if nonexistent
			...
			let local_target = world_gdp_ppp_obj[hyde_years[i]];
			
			let local_scalar = local_target/local_input_sum;
			
			GeoPNG.saveNumberRasterImage({
				file_path: `${output_folder}GDP_PPP_${hyde_years[i]}.png`,
				format: "float32",
				width: 4320,
				height: 2160,
				function: (local_index) => local_input_png.data[local_index]*local_scalar
			});
			console.log(`- ${hyde_years[i]} - Input GDP PPP: ${String.formatNumber(local_input_sum)}, Target GDP PPP: ${String.formatNumber(local_target)} - Scalar: ${local_scalar}`);
			await Blacktraffic.yield();
		}
	}
